    ...
    systemd.services.ytdownload = {
      ...
      serviceConfig = {
        ...
        Environment = [
          ...
          "PATH=${lib.makeBinPath (with pkgs; [ yt-dlp rclone (writeShellScriptBin "ytdownload" ''cd "$PWD" && ${yt-dlp}/bin/yt-dlp -o "Videos/%(title)s.%(ext)s" "$@"'') ])}:/run/current-system/sw/bin"
        ];
        ...
        ProtectHome = true;
        ReadWritePaths = [ "/var/lib/ytdownload" "/tmp" ];
      };
      
      wants = [ "ytdownload-setup.service" "ytdownload-script-setup.service" ];
    };
